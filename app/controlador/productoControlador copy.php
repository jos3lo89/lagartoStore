...
use app\modelo\principalModelo;

class productoControlador extends principalModelo
{
	...
	public function registrarProductoControlador()
	{
		...
		$color = $this->limpiarCadena($_POST['producto_color']);
		...
		if ($color != "") {
			if ($this->verificarDatos("[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,30}", $color)) {
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
	...
	public function listarProductoControlador($pagina, $registros, $url, $busqueda, $categoria)
	{
		...
		if (isset($busqueda) && $busqueda != "") {
			...
			$consulta_datos = "SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_color LIKE '%$busqueda%' ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

			$consulta_total = "SELECT COUNT(producto_id) FROM producto WHERE producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_color LIKE '%$busqueda%'";
		} elseif ($categoria > 0) {
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
			...
			foreach ($datos as $rows) {
				...
				if (is_file("./app/vista/productos/" . $rows['producto_foto'])) {
					$tabla .= '<img src="' . APP_URL . 'app/vista/productos/' . $rows['producto_foto'] . '">';
				} else {
					$tabla .= '<img src="' . APP_URL . 'app/vista/productos/default.png">';
				}
				$tabla .= '</p>
		                </figure>
		                <div class="media-content ">
		                    <div class="content ">
		                        <div class="is-flex is-flex-direction-row is-justify-content-start is-align-content-center">
									<div class="box p-2 has-background-primary"> <p>' . $contador . '</p> </div>
		                            <div class="box p-2 ml-4 has-background-warning"> <p class="has-text-link" >Nombre: <span class="has-text-black" >' . $rows['producto_nombre'] . '</span> </p>  </div>
		                            <div class="box p-2 ml-4 has-background-warning"> <p class="has-text-link" >Codigo: <span class="has-text-black" >' . $rows['producto_codigo'] . '</span> </p> </div>
		                            <div class="box p-2 ml-4 has-background-warning"> <p class="has-text-link" >Precio: <span class="has-text-black" > S/ ' . $rows['producto_precio_venta'] . '</span> </p>  </div>
		                            <div class="box p-2 ml-4 has-background-warning"> <p class="has-text-link" >Stock:  <span class="has-text-black" >' . $rows['producto_stock_total'] . '</span> </p> </div>
		                            <div class="box p-2 ml-4 has-background-warning"> <p class="has-text-link" >Categ:  <span class="has-text-black" >' . $rows['categoria_nombre'] . '</span> </p> </div>
		                            <div class="box" style="display: none;"	></div>
		                        </div>
		                    </div>
		                    <div class="has-text-right">
		                        <a href="' . APP_URL . 'productoFoto/' . $rows['producto_id'] . '/" class="button is-info is-rounded is-small">
			                    	<i class="far fa-image fa-fw"></i>
			                    </a>

		                        <a href="' . APP_URL . 'productActualizar/' . $rows['producto_id'] . '/" class="button is-success is-rounded is-small">
	...
	public function eliminarProductoControlador()
	{
		...
		if ($eliminarProducto->rowCount() == 1) {
			...
			if (is_file("../vista/productos/" . $datos['producto_foto'])) {
				chmod("../vista/productos/" . $datos['producto_foto'], 0777);
				unlink("../vista/productos/" . $datos['producto_foto']);
			}
